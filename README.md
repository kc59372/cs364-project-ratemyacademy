...
For P07, we used ChatGPT as a supplemental debugging and design aid while working on displaying database reviews as cards, updating the /api/search query to match schema changes, and troubleshooting the write-review flow after normalizing review data with course_id and professor_id. We used the tool to help reason through SQL joins, frontend fetch/rendering behavior, browser DevTools payload inspection, and route/schema mismatches; we made the implementation decisions and code changes myself and used the AI assistance to clarify errors and verify my understanding. Also for P07, we used ChatGPT as a supplemental debugging and design aid while refining the normalized review workflow, including diagnosing why initialized reviews displayed twice, reasoning through SERIAL IDs and seed data behavior, checking section seed inserts that use course lookups, and designing a database-driven section dropdown for the write-review form. We used the tool to review schema/query alignment, frontend dropdown population, /api/sections route behavior, and review submission flow; we made the actual implementation choices and code edits myself while using AI guidance to understand issues and verify the direction.
